...
func stageArchiveFile(fileName, localArtPath string, art *wfv1.Artifact) (string, string, error) {
	...
	if strat.None != nil {
		// TODO: this is inefficient because *just* tar.gz'ed the file and now we are untaring it,
		// so we are wasting effort. We should change the implementations of
		// RuntimeExecutor.CopyFile so that tgz happens later. This requires changing both the
		// docker executor and the kubelet executor
		log.Info("Disabling archive mode before upload")
		...
		if isDir {
			...
			// minio client can infer its mime-type, based on file extension, and so that the
			// original filename is incorporated into the final path.
			localArtPath = path.Join(tempOutArtDir, fileName)
